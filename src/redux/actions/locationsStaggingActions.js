...
export const findProvinces = () => async (dispatch) => {
  try {
    ...
    const res = await HttpRequest.get('/staging/api/locations/provinces');
...
export const findDistricts = (data) => async (dispatch) => {
  try {
    ...
    const res = await HttpRequest.get(
      `/staging/api/locations/districts/${data.province_id}`
    );
...
export const findSectors = (data) => async (dispatch) => {
  try {
    dispatch(creator(FIND_LOCATION_START));
    const res = await HttpRequest.get(`/staging/api/locations/sectors/${data.district_id}`);
...
export const findCells = (data) => async (dispatch) => {
  try {
    dispatch(creator(FIND_LOCATION_START));
    const res = await HttpRequest.get(`/staging/api/locations/cells/${data.sector_id}`);
...
export const findVillages = (data) => async (dispatch) => {
  try {
    dispatch(creator(FIND_LOCATION_START));
    const res = await HttpRequest.get(`/staging/api/locations/villages/${data.cell_id}`);

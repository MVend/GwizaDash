...
export const createGroup = (data) => async (dispatch) => {
  try {
    ...
    const res = await HttpRequest.post('/staging/api/groups', data);
...
export const deleteGroup = (group_id) => async (dispatch) => {
  try {
    const res = await HttpRequest.delete(`/staging/api/groups/${group_id}`);
    toast.success(res.message);
...
export const findAll = (data) => async (dispatch) => {
  try {
    ...
    const res = await HttpRequest.get(`/staging/api/groups?page=${data.page}&size=${data.size}`);
...
export const search = (data) => async (dispatch) => {
  try {
    ...
    const res = await HttpRequest.get(
      `/staging/api/groups/search?searchHint=${data.searchHint}&page=${data.page}&size=${data.size}`,
    );
    dispatch(creator(GET_GROUPS_SUCCESS, res.data));
  } catch (e) {
    if (e.response && e.response.data) {
...
export const findOne = (data) => async (dispatch) => {
  try {
    ...
    const res = await HttpRequest.get(`/staging/api/groups/${data.group_id}`);
...
export const updateGroup = (data, group_id) => async (dispatch) => {
  try {
    ...
    const res = await HttpRequest.put(`/staging/api/groups/${group_id}`, data);
    toast.success(res.message);
...
export const upload = (data) => async (dispatch) => {
  try {
    ...
    await HttpRequest.post('/staging/api/groups/upload/', data);

    const res = await HttpRequest.get('/staging/api/groups?page=0&size=1000');
    dispatch(creator(GET_GROUPS_SUCCESS, res.data));

...
export const findAll = (data) => async (dispatch) => {
  try {
    ...
    const res = await HttpRequest.get(
      `/staging/api/members?group_id=${data.group_id}&page=${data.page}&size=${data.size}`
    );
...
export const findOne = (data) => async (dispatch) => {
  try {
    dispatch(creator(GET_MEMBERS_START));
    const res = await HttpRequest.get(`/staging/api/members/${data.member_id}`);
...
export const createMember = (data) => async (dispatch) => {
  try {
    ...
    const res = await HttpRequest.post('/staging/api/members', data);
...
export const deleteMember = (member_id) => async (dispatch) => {
  try {
    const res = await HttpRequest.delete(`/staging/api/members/${member_id}`);
    toast.success(res.message);
...
export const updateMember = (data, member_id) => async (dispatch) => {
  try {
    dispatch(creator(CREATE_MEMBER_START));
    const res = await HttpRequest.put(`/staging/api/members/${member_id}`, data);
    toast.success(res.message);
...
export const search = (data) => async (dispatch) => {
  try {
    dispatch(creator(GET_MEMBERS_START));
    const res = await HttpRequest.get(
      `/staging/api/members/search?group_id=${data.group_id}&searchHint=${data.searchHint}&page=${data.page}&size=${data.size}`
    );
    dispatch(creator(GET_MEMBERS_SUCCESS, res.data));
  } catch (e) {
    if (e.response && e.response.data) {
...
export const upload = (data, group_id) => async (dispatch) => {
  try {
    ...
    const res = await HttpRequest.post(
      `/staging/api/members/upload/${group_id}`,
      data
    );
    toast.success(res.message);
